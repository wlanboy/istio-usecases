...
```bash
./uninstall.sh                   # Standard-Namespace: ratelimit-demo
./uninstall.sh mein-namespace    # eigenen Namespace verwenden
```

Entfernt nur die von diesem Usecase angelegten Ressourcen (EnvoyFilter,
Deployment, Service, ConfigMap) — der Namespace selbst bleibt bestehen, da er
z.B. gemeinsam mit `ratelimitService/` genutzt werden könnte. Soll auch der
Namespace komplett verschwinden:
